...
void* thread_convolute(void* arg) {
    ...
    int row, pix, bit;
    for (row = td->start_row; row < td->end_row; row++) {
        for (pix = 0; pix < td->srcImage->width; pix++) {
            for (bit = 0; bit < td->srcImage->bpp; bit++) {
...
void convolute(Image* srcImage, Image* destImage, Matrix algorithm) {
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        ...
        threadData[i].end_row = (i == NUM_THREADS - 1) ? srcImage->height : (i + 1) * rows_per_thread;
...
int main(int argc, char** argv) {
    ...
    if (!strcmp(argv[1], "pic4.jpg") && !strcmp(argv[2], "gauss")) {
        printf("You have applied a gaussian filter to Gauss which has caused a singularity.\n");
    }
